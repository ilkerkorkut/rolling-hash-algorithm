...
GITHUB_USERNAME ?= $(shell read -p "Your Github username: " github_username; echo $$github_username)
PROJECT_NAME ?= $(shell read -p "'Kebab-cased' Project Name: " project_name; echo $$project_name)
PROJECT_NAME_PASCAL_CASE ?= $(shell read -p "'Pascal-cased' Project Name: " project_name_pascal_case; echo $$project_name_pascal_case)
PROJECT_NAME_CAMEL_CASE ?= $(shell read -p "'Camel-cased' Project Name: " project_name_camel_case; echo $$project_name_camel_case)
prepare-initial-project:
	grep -rl ilkerkorkut . --exclude=README.md --exclude-dir=.git --exclude-dir=.idea --exclude-dir=.vscode | xargs sed -i 's/ilkerkorkut/$(GITHUB_USERNAME)/g'
	echo ""
